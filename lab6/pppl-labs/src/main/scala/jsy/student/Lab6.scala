...
object Lab6 extends jsy.util.JsyApplication {
   ...
   * Jake Traut
   *
   * Partner: Brent Fosdick
   * Collaborators: Steven Conflenti, Cameron Taylor, Jennifer Michael, Alex Doran, Loic Guegan
   */
  ...
  object REParser extends Parsers {
    ...
    def union(next: Input): ParseResult[RegExpr] = intersect(next) match {
      case Success(r, next) => {
        def unions(acc: RegExpr, next: Input): ParseResult[RegExpr] =
          if (next.atEnd) Success(acc, next) //why throw success
          else (next.first, next.rest) match {
            case ('|', next) => intersect(next) match {
              case Success(r, next) => unions(RUnion(acc, r), next) //why throw success
              case _ => Failure("expected intersect", next)  //why throw failure - need an intersect after '|' but got nothing or invalid expr
            }
            case _ => Success(acc, next) //why throw success - epsilon case, first block successful move on to rest
          }
        unions(r, next) // RSingle(a),next: "|c"
      }
      case _ => Failure("expected intersect", next) //why throw failure
    }

   // intersect ::= concat {& concat}
    def intersect(next: Input): ParseResult[RegExpr] = concat(next) match {
      case Success(r,next) => {
        def intersects(acc: RegExpr,next: Input): ParseResult[RegExpr] = //0 or more
          if (next.atEnd) Success(acc,next)
          else (next.first,next.rest) match {
            case('&',next) => concat(next) match {
              case Success(r,next) => intersects(RIntersect(acc,r),next)
              case _ => Failure("expected concat",next) //looking for a concat but got none
            }
            case _ => Success(acc,next) //epsilon case
          }
        intersects(r,next)
      }
   ...
   // not ::= ~ not | star
    def not(next: Input): ParseResult[RegExpr] = (next.first, next.rest) match {
      //first case is ~ not
      case ('~', rest) => not(rest) match { //if there is a neg, run not on rest
        case Success(r,next) => Success(RNeg(r),next) //if successful then return success
        case fail => fail
      }
      //2nd case is star
      case (_,_) => star(next) match {
    ...
    def star(next: Input): ParseResult[RegExpr] = atom(next) match {
      case Success(r,next) => {
        def stars(acc: RegExpr, next: Input): ParseResult[RegExpr] =
          if (next.atEnd) Success(acc,next)
          else (next.first, next.rest) match { //all possible cases for star
   ...
   // atom ::= ! | # | c | . | ( re )
    def atom(next: Input): ParseResult[RegExpr] = {
      if (next.atEnd) Failure("empty and no match", next)
      else (next.first, next.rest) match {
  ...
  def retest(re: RegExpr, s: String): Boolean = {
    //Spec: if a prefix of chars matches re, then return sc(suffix of chars). otherwise false
    def test(re: RegExpr, chars: List[Char], sc: List[Char] => Boolean): Boolean = (re, chars) match {
      /* Basic Operators */
      case (RNoString, _) => false
      case (REmptyString, _) => sc(chars) //sc = {chars => chars.isEmpty}
      case (RSingle(_), Nil) => false //Nil cant match character
      case (RSingle(c1), c2 :: t) => if (c1 == c2) sc(t) else false //only looking for the character c
      case (RConcat(re1, re2), _) => test(re1, chars, {next => test(re2, next, sc)}) //check if valid expr
      case (RUnion(re1, re2), _) => test(re1, chars, sc) || test(re2, chars, sc) //OR case
      case (RStar(re1), _) => sc(chars) || test(re1, chars, { //test for 0 case OR 1 or more
        next => if (next.size >= chars.size) false else test(RStar(re1),next,sc)})

      /* Extended Operators */
      case (RAnyChar, Nil) => false
      case (RAnyChar, _ :: t) => sc(t)
      case (RPlus(re1), _) => test(RConcat(re1, RStar(re1)), chars, sc) //1 or more
      case (ROption(re1), _) => if (sc(chars)) true else test(re1, chars, sc) //0 or 1

      /** *** Extra Credit Cases *****/
      case (RIntersect(re1, re2), _) => test(re1,chars,sc) && test(re2,chars,sc)
      case (RNeg(re1), _) => if (chars == re1) false else true //needs a test recursion here
    }
