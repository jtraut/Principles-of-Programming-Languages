...
object Lab4 extends jsy.util.JsyApplication {
  ...
  def typeInfer(env: TEnv, e: Expr): Typ = {
    ...
    e match {
      ...
      case Function(p, params, tann, e1) => { //e1 is body
        // Bind to env1 an environment that extends env with an appropriate binding if
        // the function is potentially recursive.
        //env {string name -> type}
        val env1 = (p, tann) match {
          case (Some(f), Some(tret)) =>
            val tprime = TFunction(params, tret)
            extend(env, f, tprime) //if function is recursive add it to the env
          case (None, _) => env //if not then dont update env
          case _ => err(TUndefined, e1)
        }
        // Bind to env2 an environment that extends env1 with bindings for params.
        val env2 = env1 ++ params //++ append on list to add params to env
        // Match on whether the return type is specified.
        tann match {
          case None => TFunction(params, typeInfer(env2, e1))
          case Some(tret) => {
            if (tret == typeInfer(env2, e1)) TFunction(params, tret)
            else err(typeInfer(env2, e1), e1)
          }
        }
      }
      //args = list of expr, e = type
      case Call(e1, args) => typ(e1) match {
        //checking params and args in parallel to see if type matches
        case TFunction(params, tret) if (params.length == args.length) =>
          (params, args).zipped.foreach {
            (param,arg) => (param,arg) match {
              case ((p, ptype),arg) => if (typ(arg) != ptype) err(typ(arg), arg)
            }
          };
          tret
        case tgot => err(tgot, e1)
      ...
      case GetField(e1, f) => typ(e1) match {
        ...
        case _ => err(typ(e1),e1) //e1 not an object
      }
    }
  }
  ...
  def substitute(e: Expr, v: Expr, x: String): Expr = {
    ...
    e match {
      ...
      case Call(e1, args) => Call(subst(e1), args map subst)
        /***** New cases for Lab 4 */
        //example object: const b = 3 {f: 3, d: 2}. f = 3
      ...
      case Call(v1, args) if isValue(v1) && (args forall isValue) =>
        v1 match {
          case Function(p, params, _, e1) => {
            val e1p = (params, args).zipped.foldRight(e1){ //need to subst all args e[Vi/Xi] e1p e1 with all param/val added in
